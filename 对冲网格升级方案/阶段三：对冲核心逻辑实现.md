...
#### 新建文件：`src/gridbot/hedge_pair_manager.py`

````python path=src/gridbot/hedge_pair_manager.py mode=EDIT
"""
...
```bash
# 1. 单元测试对冲配对逻辑
pytest tests/unit/gridbot/test_hedge_pair_manager.py -v
...
print('✅ 对冲配对创建成功')
print(f'配对ID: {pair.pair_id}, 价格: {pair.price}, 状态: {pair.hedge_status}')
"
```
